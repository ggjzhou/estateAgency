<template>
  <div class="box">
    ...
    <el-button type="primary" id="search" icon="el-icon-search" @click="getHouseTypeData()"></el-button>
      ...
      <div class="right">
        <el-descriptions class="margin-top" :column="1" :size="size">
          ...
          <el-descriptions-item>
            ...
            <el-tag size="small">{{ item.houseStatus }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item>
            <template slot="label">
...
export default {
  ...
  methods: {
    ...
    toHousedetail(index) {  //判断循环的时候点击的页面
      ...
      this.$session.set("selectHouseData", this.selectHouseData); //存session
    ...
    getHouseTypeData() {
      if (this.input != '' && this.input != undefined) {
        this.$axios({
          url: "/house/queryByType/" + this.input,
          methods: "GET",
          header: {
            "Content-Type": "application/json;charset=UTF-8",
          },
        }).then((res) => {
          this.houseTypeData = res.data.data;  //给housetypedata赋值
          this.houses = this.houseTypeData;
        });
      } else {
        this.$axios({
    ...
    siftHouse() {
      // this.houses = this.houseTypeData
      let houses = this.houseTypeData
      if (this.price == 2) {
        houses = this.houseTypeData.filter((h) => {
